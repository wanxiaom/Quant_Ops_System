...
try:
    ...
    print('无法导入Connector模块，请确保py_connect.cp3xx-win_amd64.pyd,(3xx为相应python版本)文件在当前目录下')
    exit()
class DB_Api():
    ...
    def close(self):
        '''主动关闭连接'''
        if hasattr(self, '_DB_Api__conn') and self.__conn is not None:
            self.__conn.close()
            self.__conn = None
    ...
    def stock_EOD_price(self,codes:list,date0: str='yyyymmdd', date1: str='yyyymmdd',data_names:list=[])->pd.DataFrame:
        '''
        提取个股的日频价格数据
        :param codes: 个股代码，数据类型为list。
        :param date0: 日期，数据类型为yyyymmdd格式的字符串。
        :param date1: 日期，数据类型为yyyymmdd格式的字符串。
        :param data_names: 数据类型为list，为空时提取所有字段
        :return: DataFrame
        '''
        func_name, kwargs = self.__get_kwargs(sys._getframe(), locals().items())
        data = self.__conn.get_data_remote(func_name, kwargs)
        ...
        if data.empty:
            return data
        data['交易日期'] = data['交易日期'].map(lambda t: pd.Timestamp(t, unit='ms'))
        return data

    def stock_EOD_indicator(self,codes:list,date0: str='yyyymmdd', date1: str='yyyymmdd',data_names:list=[])->pd.DataFrame:
        '''
        提取个股的日频衍生数据
        :param codes: 个股代码，数据类型为list。
        :param date0: 日期，数据类型为yyyymmdd格式的字符串。
