...
def download(trade_date: str, output_dir: str, force: bool = False):
    """
    按天获取所有股票的数据并保存为 Parquet 文件
    :param trade_date: 交易日期 (str)
    :param output_dir: 输出目录 (str)
    :param force: 是否强制重新拉取 (bool)
    """
    year = trade_date[:4]
    ...
    try:
        ...
        df = df.rename(columns={'PREMIUMRATE_AH': 'ah_premium_rate'})
        df = df.dropna(subset=['ah_premium_rate'])
        cols = ['code', 'date', 'ah_premium_rate']
        df = df[cols]
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="SOP-compliant Downloader for AH Premium.")
